...
담당·통합자는 **A=영원 · B=서정 · C=미리(통합자 겸임)** 으로 확정됐다. 각자 `OWNERS.md` 「담당이 정해지면 할 일」 3단계(deny·확인·훅)를 아직 안 켰다면 지금 켠다.
...
**팀 결정 대기 5건은 SPEC 12절에서 2026-08-19에 확정됐다.** F11(TTS)은 Should로 상향 · F16 오답 처리는 "설명 후 다음 문제"로 확정 · **은행명은 가상 은행 `한걸음은행`(정답)·`새봄은행`·`푸른은행`(선택지)으로 확정** · **비밀번호 단계는 이번 MVP 제외로 확정** · **목표 금액은 홈 화면에 한 번만 "오늘의 미션 — 김민수에게 50,000원 보내기" 표시로 확정**. 아래 Task의 `국민은행`/`신한은행`/`농협은행`은 이 확정에 맞춰 가상 은행명으로 바꿔 적었다.
...
- [ ] A2.1 `출금계좌` 라벨 + 가짜 계좌번호 `한걸음은행 123456-78-901234` (SPEC 12.3 확정 — 가상 은행명)
- [ ] A2.1b **홈 화면에 한 번만** `오늘의 미션 — 김민수에게 50,000원 보내기` 표시 (`missions.json` 의 `title`). 각 단계 화면에는 반복하지 않는다 — F10(Won't)과 구분된다 (SPEC 12.5 확정)
- [ ] A2.2 `잔액 1,250,000 원`
...
- [ ] B1.2 `bank` `wrong` — 김민수님 은행이 한걸음은행이라고 (SPEC 12.3 확정)
...
- [ ] S1.4 눈 확인 — 은행에서 "새봄은행": 화면 그대로 + 한걸음은행 안내
...
| C | C5 음성 낭독 (세부 Task 미작성) | C | F11 | Should | ☐ |
| S3 | 시연 리허설 | 셋이 | — | — | ☐ |
...
- **F9 반복 오답 힌트 상승 / F10 단계별 안내 배너** — SPEC 10절 Won't. 만들지 않는다.
  - **F10 ≠ F17.** F17(진행 표시)은 만든다. F10은 "무엇을 하세요"라는 큰 지시 배너이고, F17은 "2 / 5 단계 — 계좌번호 입력"이라는 작은 위치 표시다 (SPEC 4.1).
- **F11 음성 낭독(TTS)** — **Should로 확정**(SPEC 12.1). C4(마이크)와 같은 Should 단계에서, C 담당이 답변 출력 지점(`showAnswer` 호출부)에 `speechSynthesis` 를 붙이는 Task가 추가된다. **Must 레인(S0~S2)이 끝나기 전까지는 착수하지 않는다.** Should 착수 시 `.claude/agents/code-reviewer.md` 의 `speechSynthesis` 무조건 지적 규칙도 같이 고친다.
...
- **6자리 비밀번호 단계** — **SPEC 12.4 확정: 이번 MVP 제외.** **SOL 쉬운이체에도 없다**(화면 확인 · SPEC 13.5 정정 2) — 지금 흐름이 레퍼런스와 같다.
- **목표 금액 화면 표시** — **SPEC 12.5 확정: 홈 화면에 한 번만** `오늘의 미션 — 김민수에게 50,000원 보내기` 표시(A2.1b). 각 단계 화면에는 반복하지 않는다.
- **F16 오답 재시도** — **SPEC 12.2 확정: 설명 후 다음 문제.** B5.5·B5.6대로 구현한다. 다시 고를 기회를 주지 않는다. B 담당.
